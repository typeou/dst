...
local wincounter = 0.0
local wolfcounter = 0.0
local waxcounter = 0.0
local wilcounter = 0.0
local wickcounter = 0.0
local woodcounter = 0.0
local wencounter = 0.0

local wintalk = false
local wolftalk = false
local waxtalk = false
local wiltalk = false
local wicktalk = false
local woodtalk = false
local wentalk = false

local function winonastart(inst)
	if wintalk then
		wintalk = false
	else
		wintalk = true
...
local function wolfgangstart(inst)
	if wolftalk then
		wolftalk = false
	else
		wolftalk = true
...
local function waxwellstart(inst)
	if waxtalk then
		waxtalk = false
	else
		waxtalk = true
...
local function wilsonstart(inst)
	if wiltalk then
		wiltalk = false
	else
		wiltalk = true
...
local function wickerbottomstart(inst)
	if wicktalk then
		wicktalk = false
	else
		wicktalk = true
...
local function woodiestart(inst)
	if woodtalk then
		woodtalk = false
	else
		woodtalk = true
...
local function wendystart(inst)
	if wentalk then
		wentalk = false
	else
		wentalk = true
...
local function firststart(inst)
	wintalk = true
	wolftalk = true
	waxtalk = true
	wiltalk = true
...
local function secondstart(inst)
	wintalk = true
	wicktalk = true
	woodtalk = true
	wentalk = true
...
local function musicaura(inst)
	for k,v in pairs(AllPlayers) do
		if v.prefab == "winona" and wintalk == true then
			if wincounter == 0 then
				v.components.talker:Say("")
			end
		elseif v.prefab == "wolfgang" and wolftalk == true then
			if wolfcounter == 1.25 then
				v.components.talker:Say("")
			end
		elseif v.prefab == "waxwell" and waxtalk == true then
			if waxcounter == 4.25 then
				v.components.talker:Say("")
			end
		elseif v.prefab == "wilson" and wiltalk == true then
			if wilcounter == 9  then
				v.components.talker:Say("")
			end
		elseif v.prefab == "wickerbottom" and wicktalk == true then
			if wickcounter == 6 then
				v.components.talker:Say("")
			end
		elseif v.prefab == "woodie" and woodtalk == true then
			if woodcounter == 3.5 then
				v.components.talker:Say("")
			end
		elseif v.prefab == "wendy" and wentalk == true then
			if wencounter == 3  then
				v.components.talker:Say("")
			end
		end
	end
	wincounter = wincounter + 0.25
	wolfcounter = wolfcounter + 0.25
	waxcounter = waxcounter + 0.25
	wilcounter = wilcounter + 0.25
	wickcounter = wickcounter + 0.25
	woodcounter = woodcounter + 0.25
	wencounter = wencounter + 0.25
	if wincounter >= 2.75  then
		wincounter = 0.0
	elseif wolfcounter >= 4.25 then
		wolfcounter = 0.0
	elseif waxcounter >= 9 then
		waxcounter = 0.0
	elseif wilcounter > 9 then
		wilcounter = 0.25
	elseif wickcounter >= 9.75 then
		wickcounter = 0.0
	elseif woodcounter >= 9 then
		woodcounter = 0.0
	elseif wencounter >=  9.5 then
		wencounter = 0.0
	end
end
...
local master_postinit = function(inst)
	...
	inst:DoPeriodicTask(0.25,musicaura)
	inst:ListenForEvent("winona",winonastart)
	inst:ListenForEvent("wolfgang",wolfgangstart)
	inst:ListenForEvent("waxwell",waxwellstart)
	inst:ListenForEvent("wilson",wilsonstart)
	inst:ListenForEvent("wickerbottom",wickerbottomstart)
	inst:ListenForEvent("woodie",woodiestart)
	inst:ListenForEvent("wendy",wendystart)
	inst:ListenForEvent("first",firststart)
	inst:ListenForEvent("second",secondstart)
	inst:ListenForEvent("stopall",stopall)
